...
const ntr = document.querySelectorAll(".ntr .btnVal");
...
for (const elm of ntr) {
  elm.parentElement.children[1].onclick = () => ntrClick(elm, 1.1);
  elm.parentElement.children[2].onclick = () => ntrClick(elm, 0.9);
}
function ntrClick(elm, value) {
  if (elm.value == value) {
    elm.value = 1;
  } else {
    for (const elm of ntr) {
      if (elm.value == value) {
        elm.value = 1;
        outputSts(Array.prototype.indexOf.call(elm.parentElement.parentElement.children, elm.parentElement));
      }
    }
    elm.value = value;
  }
...
function ntrBtnBgClr() {
  for (const child of ntr) {
    const elm = child.parentElement;
    elm.children[1].style.backgroundColor = "ButtonFace";
    elm.children[1].style.fontWeight = "unset";
    elm.children[2].style.backgroundColor = "ButtonFace";
    elm.children[2].style.fontWeight = "unset";
    switch (+elm.children[0].value) {
      case 1.1:
        elm.children[1].style.backgroundColor = "red";
        elm.children[1].style.fontWeight = "bold";
        break;
      case 0.9:
        elm.children[2].style.backgroundColor = "blue";
        elm.children[2].style.fontWeight = "bold";
        break;
    }
  }
}
...
for (let index = 0; index < 6; index++) {
  sts[index].onchange = () => {
    if (index === 0)
      ...
      ev[index].value = (Math.ceil((Math.ceil(sts[index].value / ntr[index - 1].value) - 5) * 100 / lv.value) - bs[index].value * 2 - iv[index].value) * 4;
    outputRslt(index);
    if (+ev[index].value < -4) {
...
function outputSts(index) {
  ...
  if (index === 0)
    ...
    sts[index].value = Math.floor((base + 5) * ntr[index - 1].value);
  outputRslt(index);
}
...
copyBtn.onclick = () => {
  new Promise((resolve) => {
    ...
    .then(() => copyBtnClick("コピーしました。"))
    .catch(() => copyBtnClick("コピーに失敗しました。"))
  function copyBtnClick(msgStr) {
    const copyText = document.createElement("div");
    copyText.setAttribute("class", "copyText");
    copyText.appendChild(document.createTextNode(msgStr));
    document.querySelector("#base").appendChild(copyText);
    setTimeout(() => {
      copyText.remove();
    }, 1400);
  }
