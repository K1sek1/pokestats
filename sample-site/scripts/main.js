...
function ntrClick(elm, index) {
  ...
  if (elm.dataset.value == value) {
    ...
    elm.children[index].value = value;
    elm.children[1 - index].value = 1;
  }
  outputSts(Array.prototype.indexOf.call(ntr, elm) + 1);
}
...
copyBtn.onclick = () => {
  new Promise((resolve) => {
    ...
    .then(() => {
      ...
      copyText.appendChild(document.createTextNode("コピーしました。"))
    ...
    .catch(() => {
      const copyText = document.createElement("div");
      copyText.setAttribute("class", "copyText");
      copyText.appendChild(document.createTextNode("コピーに失敗しました。"))
      document.querySelector("#base").appendChild(copyText);
      setTimeout(() => {
        copyText.remove();
      }, 1400);
